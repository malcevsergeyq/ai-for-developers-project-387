...
pull request, ревью и регулярные проверки по расписанию — как это устроено и как вызывать
агента, описано в [`AGENT-PROCESS.md`](./AGENT-PROCESS.md). План развития — [`BACKLOG.md`](./BACKLOG.md).
...
Фронт и бэкенд в деве живут на разных адресах, поэтому фронту нужно знать адрес API.
Он берётся из переменной `VITE_API_URL`, и для дев-режима она уже задана в
[`ui/.env.development`](./ui/.env.development) — `http://127.0.0.1:3000`. Менять её нужно
только если бэкенд поднят на другом порту или вместо него используется мок (`npm run mock`
на `:4010`). Шаблон для своих значений — `ui/.env.example`.

> Адрес именно `127.0.0.1`, а не `localhost`: на Windows `localhost` резолвится в `::1`,
> а сервер слушает IPv4 — запросы уйдут в никуда.

Если `VITE_API_URL` потерялась, запрос за данными остаётся у Vite, тот отдаёт на него
`index.html`, и в интерфейсе появляется «Сервер ответил не в формате JSON». Проверить всю
связку одной командой: `npm run test:e2e:dev` (свои дев-серверы перед этим надо остановить —
прогон поднимает свои на тех же портах).

В контейнере и на деплое переменная не нужна: там фронт и API отдаёт один процесс с одного
origin, и `VITE_API_URL` при сборке образа пустой (`Dockerfile`).
...
проверки Hexlet). На продакшене засев тоже включён намеренно: публичная ссылка — витрина,
и в пустом календаре записаться не на что. Выключается переменной `SEED_DEMO=false`
в настройках сервиса (см. комментарий в `render.yaml`). Принимаются `true/1/yes/on` и `false/0/no/off`; любое другое
...
| `npm run test:e2e:dev` | smoke-проверка дев-связки из этого README (`:3000` + `:5173`) |
| `npm run lint` | ESLint |
